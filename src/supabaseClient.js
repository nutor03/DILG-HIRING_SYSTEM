...
const SUPABASE_ANON = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJ6aWFwdmxiYnhqcnl1aW1xb2xvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI3NjM5MjgsImV4cCI6MjA4ODMzOTkyOH0.GoCLLjqwPV40h_nF9j54g38Zr280jQqqKv1a_owlhwo";                        // ← replace
...
function buildUserObject(user, meta) {
  return {
    ...
    role:      user.email === "admin@dilg.gov.ph" ? "admin" : "user",
  };
}
...
export async function submitApplication(app) {
  const { error } = await supabase.from("applications").insert([{
    ...
    edu_data:        app.eduData,
    work_data:       app.workData,
    file_name:       app.fileName || null,
  }]);
  if (error) throw friendlyError(error);
}
...
function normalizeApp(row) {
  return {
    ...
    eduData:        row.edu_data   || {},
    workData:       row.work_data  || {},
